...
-(UILabel *)contentTitle{
    if (!_contentTitle) {
        ...
        contentTitle.text = [AppDelegate getURLWithKey:@"ningdewenti"];
...
-(UITextField *)topicTextfield{
    if (!_topicTextfield) {
        ...
        cellTextfield.placeholder = [AppDelegate getURLWithKey:@"ningdezhuti"];
        //内容对齐方式
...
-(UITextView *)contentTextView{
    if (!_contentTextView) {
        ...
        placeHolderView.text = [AppDelegate getURLWithKey:@"ningdeyijian"];
...
-(UILabel *)reindLabel{
    if (!_reindLabel) {
        ...
        reindLabel.text = [NSString stringWithFormat:@"      %@",[AppDelegate getURLWithKey:@"ningdeyijianshuoming"]];
...
-(UIButton *)submitBtn{
    if (!_submitBtn) {
        ...
        [button setTitle:[AppDelegate getURLWithKey:@"wentitijiao"] forState:UIControlStateNormal];
...
-(void)selectSource:(UIButton *)sender{
    if (!self.topicTextfield.text.length || !self.contentTextView.text.length) {
        [[PromptBox sharedBox] showPromptBoxWithText:[AppDelegate getURLWithKey:@"shurufankui"] onView:self.view hideTime:2 y:0];
        return;
    }
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.title = [AppDelegate getURLWithKey:@"XiangzengFankui"];
}
