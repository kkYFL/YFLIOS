...
+(CGFloat)CellHWithModel:(LearningTaskModel *)model{
    NSString *content = [NSString stringWithFormat:@"%@%@",[NSString stringWithFormat:@"%@：",[AppDelegate getURLWithKey:@"RenWujianjie"]],model.taskSummary];
...
-(void)setModel:(LearningTaskModel *)model{
    ...
    if (_model) {
        ...
        self.mingchengLabel.text = [NSString stringWithFormat:@"%@:%@",[AppDelegate getURLWithKey:@"renwumingcheng"],_model.taskTitle];
        NSString *taskTypeStr = @"";
        if ([_model.taskType integerValue] == 1) {
            taskTypeStr = [AppDelegate getURLWithKey:@"wenzi"];
        }else if ([_model.taskType integerValue] == 2){
            taskTypeStr = [AppDelegate getURLWithKey:@"shiping"];
        }else if ([_model.taskType integerValue] == 3){
            taskTypeStr = [AppDelegate getURLWithKey:@"yingping"];
        }else if ([_model.taskType integerValue] == 4){
            taskTypeStr = [AppDelegate getURLWithKey:@"tupian"];
        }
        
        self.leixingLabel.text = [NSString stringWithFormat:@"%@：%@",[AppDelegate getURLWithKey:@"renwuleixing"],taskTypeStr];
        self.learnTimeLabel.text = [NSString stringWithFormat:@"%@：%@%@",[AppDelegate getURLWithKey:@"xuexishixhang"],_model.learnTime,[AppDelegate getURLWithKey:@"fenzhogn"]];
        self.createTimeLabel.text = [NSString stringWithFormat:@"%@：%@",[AppDelegate getURLWithKey:@"chaugnjianshijian"],_model.createTime];
        //RenWujianjie
        NSString *content = [NSString stringWithFormat:@"%@:%@",[AppDelegate getURLWithKey:@"chaugnjianshijian"],_model.taskSummary];
        NSMutableAttributedString *attri = [EducationJianjieCell getAttriHeightwithString:content Speace:4.0f withFont:[UIFont systemFontOfSize:14.0f]];
